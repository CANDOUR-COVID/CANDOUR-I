...
WTP_vaccine_ppp <- tibble()
WTP_taxes_ppp <- tibble()
WTP_ticket_ppp <- tibble()
rnames <- c()
...
for (country_name in c("UK", "Australia", "Brazil", "Canada", "Colombia", "Chile",
  ...
  WTP_vaccine_ppp <- rbind(WTP_vaccine_ppp, wtp_regression(wtp_data, country_name, PPP = T)$vaccine)
  WTP_taxes_ppp <- rbind(WTP_taxes_ppp, wtp_regression(wtp_data, country_name, PPP = T)$taxes)
  WTP_ticket_ppp <- rbind(WTP_ticket_ppp, wtp_regression(wtp_data, country_name, PPP = T)$ticket)
  rownames(WTP_vaccine_ppp) <- rnames
  rownames(WTP_taxes_ppp) <- rnames
  rownames(WTP_ticket_ppp) <- rnames
}

WTP_vaccine_ppp <- WTP_vaccine_ppp %>% 
  rownames_to_column() %>%
...
for (country_name in c("UK", "Australia", "Brazil", "Canada", "Colombia", "Chile",
                       ...
                       "US")) {
  rnames <- c(rnames, country_name)
  WTP_vaccine_lc <- rbind(WTP_vaccine_lc, wtp_regression(wtp_data, country_name, PPP = F)$vaccine)
  WTP_taxes_lc <- rbind(WTP_taxes_lc, wtp_regression(wtp_data, country_name, PPP = F)$taxes)
  WTP_ticket_lc <- rbind(WTP_ticket_lc, wtp_regression(wtp_data, country_name, PPP = F)$ticket)
  rownames(WTP_vaccine_lc) <- rnames
  rownames(WTP_taxes_lc) <- rnames
  rownames(WTP_ticket_lc) <- rnames
}

WTP_vaccine_lc <- WTP_vaccine_lc %>% 
  rownames_to_column() %>%
  mutate(median = round(median),
         mean = round(mean)) %>%
  pivot_longer(-rowname, 'variable', 'value') %>%
...
stargazer(WTP_vaccine_lc, 
          summary = FALSE, 
          out = "tables/WTP_vaccine_lc.tex", 
          rownames = F, 
          title = "Willingness to pay for a COVID-19 vaccine (Local Currency)")

stargazer(WTP_taxes_lc,
          summary = FALSE, 
          out = "tables/WTP_taxes_lc.tex",
          rownames = F,
          title = "Willingness to pay extra taxes for a COVID-19 vaccine (Local Currency)")

stargazer(WTP_ticket_lc, 
          summary = FALSE,
          out = "tables/WTP_ticket_lc.tex",
          rownames = F,
          title = "Willingness to pay an extra pet ticket ticket for a COVID-19 vaccine (Local Currency)")
