...
# PRELIMINARIES ----------------------------------------------------------
country_dirs <- list.files("D:/wave_I", full.names = T) |>
  str_subset("other", negate = T)

latest_files <- map_chr(country_dirs, function(dir) {
  files <- list.files(dir, pattern = "\\.csv$", full.names = TRUE)
  files[which.max(file.info(files)$mtime)]
})

names(latest_files) <- country_dirs |> str_remove("D:/wave_I/")
...
data_CHL <- read_csv(latest_files["chile"])[-c(1:2),]
...
data_CHL <- data_CHL %>%
  mutate(RecipientEmail = str_to_lower(RecipientEmail)) |>
  arrange(StartDate) %>%
...
chile_pool <- list.files("E:/chile", full.names = T) |>
  tail(1) |>
  read_csv()
...
data_CHL <- data_CHL |>
  mutate(RecipientEmail = str_to_lower(RecipientEmail)) |>
  left_join(
    chile_pool |> mutate(email = str_to_lower(email)) |> select(email, psid),
...
na_psid <- data_CHL |>
  ...
  select(-psid) |>
  left_join(
    chile_pool |> mutate(email_conf = str_to_lower(email_conf)) |> select(email_conf, psid),
    by = c("RecipientEmail" = "email_conf")
  ) |>
  relocate(psid)
...
data_CHL <- data_CHL %>%
  filter(
    !Q_TerminateFlag %in% c("Screened", "QuotaMet"), 
    Status == "IP Address",
    Progress == "100", 
...
write_csv(
  data_CHL |> select(-postal_code),
  here("wave_I", "proc", "build", "01_data_clean", "data_chl.csv")
)
